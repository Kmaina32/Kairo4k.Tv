...
const CommandDeck = () => {
    // Real-time State
    const [cpuLoad, setCpuLoad] = useState<number[]>(new Array(60).fill(20));
    const [memoryUsage, setMemoryUsage] = useState<number[]>(new Array(60).fill(40));
    ...
    useEffect(() => {
        const interval = setInterval(() => {
            ...
            setCpuLoad(prev => {
                ...
                let next = last + noise;
                if (next < 10) next = 10 + Math.random() * 5;
                if (next > 90) next = 90 - Math.random() * 5;
            ...
            setMemoryUsage(prev => {
                ...
                if (next > 70) next = 70 - Math.random() * 5;
                return [...prev.slice(1), Math.round(next)];
            });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                ...
                <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-6 rounded-2xl relative overflow-hidden group hover:border-blue-500/30 transition-colors">
                    ...
                    <div className="flex items-end gap-2 mb-4">
                        <span className="text-4xl font-black text-white tracking-tighter">{cpuLoad[cpuLoad.length - 1].toFixed(0)}</span>
                        <span className="text-xs font-mono text-blue-400 mb-2">% PARALLEL</span>
                    </div>
                    {/* SVG CPU Graph */}
                    <div className="h-6 w-full relative overflow-hidden mb-2">
                        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                            <path
                                d={generatePath(cpuLoad, 24, 300)}
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="1"
                    ...
                    <div className="flex gap-0.5 h-2 w-full">
                        {[...Array(20)].map((_, i) => {
                            const latestCpu = cpuLoad[cpuLoad.length - 1];
                            return (
                                <div
                                    key={i}
                                    className={`flex-1 rounded-sm transition-all duration-100 ${(i / 20) * 100 < latestCpu
                                        ? latestCpu > 85 ? 'bg-red-500'
                                            : latestCpu > 60 ? 'bg-orange-500'
                                                : 'bg-blue-500'
                                        : 'bg-white/5'
                                        }`}
                                    style={{ opacity: (i / 20) * 100 < latestCpu ? 1 : 0.2 }}
                                />
                            );
                        })}
                    </div>
                ...
                <div className="bg-black/80 backdrop-blur-xl border border-white/10 p-6 rounded-2xl relative overflow-hidden group hover:border-purple-500/30 transition-colors">
                    ...
                    <div className="flex items-end gap-2 mb-4">
                        <span className="text-4xl font-black text-white tracking-tighter">{memoryUsage[memoryUsage.length - 1].toFixed(0)}</span>
                    ...
                    <div className="h-6 w-full relative overflow-hidden mb-2">
                        <svg className="absolute inset-0 w-full h-full" preserveAspectRatio="none">
                            <path
                                d={generatePath(memoryUsage, 24, 300)}
                                fill="none"
                                stroke="currentColor"
                                strokeWidth="1"
                    ...
                    <div className="w-full h-1.5 bg-white/10 mt-4 overflow-hidden rounded-full relative">
                        <div className="absolute top-0 left-0 h-full bg-purple-500/50 transition-all duration-300" style={{ width: `${memoryUsage[memoryUsage.length - 1]}%` }} />
