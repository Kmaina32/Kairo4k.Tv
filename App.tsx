...
const App: React.FC = () => {
  ...
  const fetchPlaylists = useCallback(async () => {
    ...
    try {
      ...
      if (allChannels.length > 0 && !allChannels.some(c => c.source === activeTab)) {
        setActiveTab(allChannels[0].source);
      }
    } catch (err) {
      setError("Feeds unreachable.");
    } finally {
      setIsLoading(false);
    }
  }, [activeTab]);
  ...
  if (isLoading) {
    return (
      <div className="h-screen w-full flex flex-col items-center justify-center bg-slate-950">
        ...
        <p className="text-white font-black tracking-widest text-xs uppercase animate-pulse">Establishing Nexus Uplink</p>
      </div>
    );
  }
  ...
  return (
    <div className="flex h-screen bg-[#020617] text-slate-100 overflow-hidden relative">
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar">
          {filteredChannels.map((channel) => (
            <button
              ...
              className={`w-full text-left p-4 flex items-center space-x-4 hover:bg-white/5 transition-all border-l-2 ${selectedChannel?.id === channel.id ? 'bg-indigo-600/10 border-indigo-500' : 'border-transparent'}`}
            >
              ...
              <div className="min-w-0">
                <p className={`text-xs font-bold truncate ${selectedChannel?.id === channel.id ? 'text-indigo-400' : 'text-slate-300'}`}>{channel.name}</p>
                <p className="text-[9px] text-slate-500 font-bold uppercase truncate mt-0.5">{channel.group}</p>
              </div>
            </button>
          ))}
        </div>
      ...
      <div className="flex-1 flex flex-col min-w-0 bg-slate-950 relative">
        <header className={`h-16 flex-shrink-0 border-b border-white/5 flex items-center justify-between px-6 bg-slate-950/50 backdrop-blur-xl z-50 transition-all ${isTheater ? 'opacity-0 h-0 pointer-events-none overflow-hidden' : ''}`}>
          <div className="flex items-center space-x-3">
            {!sidebarOpen && (
            ...
            <h2 className="text-sm font-black uppercase tracking-tight truncate max-w-[150px] sm:max-w-none">{selectedChannel?.name || 'No Signal'}</h2>
          </div>
          <button onClick={fetchPlaylists} className="p-2 text-slate-500 hover:text-white transition-all active:rotate-180 duration-500">
          ...
          {selectedChannel ? (
            <div className={`mx-auto w-full transition-all duration-700 ${isTheater ? 'max-w-full h-full flex items-center' : 'max-w-5xl space-y-8'}`}>
              <div className={`transition-all duration-700 ${isTheater ? 'w-full h-full' : 'relative rounded-3xl overflow-hidden shadow-2xl ring-1 ring-white/10'}`}>
                <VideoPlayer url={selectedChannel.url} poster={selectedChannel.logo} isTheater={isTheater} onToggleTheater={() => setIsTheater(!isTheater)} />
              </div>
              {!isTheater && (
                <div className="p-8 bg-slate-900/30 border border-white/5 rounded-3xl backdrop-blur-3xl animate-in fade-in slide-in-from-bottom-5">
            ...
            <div className="h-full flex flex-col items-center justify-center text-center opacity-20">
               <div className="w-32 h-32 bg-slate-900 rounded-[2.5rem] flex items-center justify-center mb-6 animate-pulse border border-white/5">
                 <svg className="w-16 h-16 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path d="M8.111 16.404a5.5 5.5 0 017.778 0M12 20h.01m-7.08-7.071c3.904-3.905 10.236-3.905 14.141 0M1.343 5.858c5.858-5.858 15.355-5.858 21.213 0" strokeWidth={1}/></svg>
               </div>
               <p className="text-[10px] font-black uppercase tracking-[0.4em]">Signal Acquisition Required</p>
            </div>
          )}
        </main>
          ...
          <nav className="lg:hidden fixed bottom-0 inset-x-0 h-20 bg-slate-950/95 border-t border-white/5 backdrop-blur-3xl flex items-center justify-around px-4 z-[55] shadow-[0_-10px_40px_rgba(0,0,0,0.5)]">
            {availableSources.slice(0, 4).map(source => (
              <button 
                key={source} 
                onClick={() => setActiveTab(source)}
                className={`flex flex-col items-center space-y-1 p-2 transition-all ${activeTab === source ? 'text-indigo-400' : 'text-slate-500'}`}
              >
                <div className={`w-1 h-1 rounded-full transition-all ${activeTab === source ? 'bg-indigo-500 shadow-[0_0_10px_rgba(99,102,241,1)]' : 'bg-transparent'}`} />
                <span className="text-[8px] font-black uppercase tracking-tighter">{source}</span>
              </button>
            ))}
            <button onClick={() => setSidebarOpen(true)} className="flex flex-col items-center space-y-1 p-2 text-indigo-500">
