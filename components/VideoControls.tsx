    ...
    <div className={`absolute bottom-0 inset-x-0 transition-all duration-500 pb-4 md:pb-10 px-4 md:px-12 flex justify-center ${showControls ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-4 pointer-events-none'}`}>
      ...
      <div className="glass h-14 md:h-20 w-full max-w-5xl rounded-[28px] md:rounded-[32px] flex items-center justify-between px-4 md:px-10 gap-2 border-white/10 shadow-[0_20px_50px_rgba(0,0,0,0.5)]">
        ...
        <div className="flex items-center space-x-2 md:space-x-6 flex-shrink-0">
          <button onClick={onTogglePlay} className="text-white hover:scale-110 transition-all p-2 md:p-2.5 bg-white/5 rounded-full hover:bg-orange-600 active:scale-90">
            {isPlaying ? (
          ...
          <div className="flex items-center space-x-2 md:space-x-3">
            ...
            <input
              ...
              className="w-12 md:w-20 h-1 bg-white/10 rounded-lg appearance-none cursor-pointer accent-orange-500 hover:accent-orange-400"
            />
          </div>
        </div>
        ...
        <div className="hidden lg:flex flex-col items-center flex-shrink-0">
          <div className="flex items-center space-x-1 kairo-cyber-glow">
            <span className="text-[14px] font-black uppercase tracking-[0.4em]">KAIRO</span>
            <span className="text-[14px] font-black text-white tracking-tighter decoration-orange-500 underline underline-offset-4">4K</span>
          </div>
        </div>
        ...
        <div className="flex items-center space-x-1.5 md:space-x-4">
          ...
          <div className="flex items-center space-x-1 md:space-x-2 bg-white/5 p-1 md:p-1.5 rounded-xl md:rounded-2xl border border-white/5">
            <div className="relative">
              <button
                ...
                className={`text-[8px] md:text-[10px] font-black px-2 md:px-2.5 py-1 md:py-1.5 rounded-lg md:rounded-xl transition-all ${showQualityMenu ? 'bg-orange-600 text-white' : 'text-white/40 hover:text-white'}`}
              >
              ...
              {showQualityMenu && (
                <MenuOverlay onClose={() => setShowQualityMenu(false)}>
                  <button onClick={() => { onQualityChange(-1); setShowQualityMenu(false); }} className={`text-[10px] font-black p-3 rounded-2xl text-left transition-all ${currentLevel === -1 ? 'bg-orange-600 text-white' : 'text-slate-400 hover:bg-white/5'}`}>AUTO</button>
                  {qualityLevels.map((lvl, idx) => (
                    <button key={idx} onClick={() => { onQualityChange(idx); setShowQualityMenu(false); }} className={`text-[10px] font-black p-3 rounded-2xl text-left transition-all ${currentLevel === idx ? 'bg-orange-600 text-white' : 'text-slate-400 hover:bg-white/5'}`}>
            ...
            <button
              ...
              className={`p-1 md:p-1.5 rounded-lg md:rounded-xl transition-all ${isRecording ? 'text-orange-500 animate-pulse' : 'text-white/40 hover:text-white'}`}
            >
          ...
          <div className="flex items-center space-x-1 md:space-x-3">
            ...
            <button onClick={onToggleTheater} className={`p-2 md:p-3 rounded-xl md:rounded-2xl transition-all ${isTheater ? 'bg-orange-600 text-white' : 'bg-white/5 text-white/40 hover:text-white'}`}>
