        ...
        channel
            ...
            .subscribe((status) => {
                if (status === 'SUBSCRIBED') {
                    console.log('[Nexus Chat] Connected to encrypted channel');
                }
            });
        ...
        <div className="flex flex-col h-full glass rounded-[40px] border-white/5 overflow-hidden">
            <div className="p-6 border-b border-white/5 flex items-center justify-between">
                <div className="flex items-center gap-3">
                    ...
                    <h3 className="text-xs font-black uppercase tracking-[0.2em] text-slate-400">Operator Comms</h3>
                </div>
                <span className="text-[8px] font-mono text-white/20 uppercase tracking-widest">Encrypted Bridge</span>
            </div>
            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-4 no-scrollbar">
                {messages.length === 0 ? (
                    <div className="h-full flex flex-col items-center justify-center opacity-20 text-center space-y-2">
                        ...
                        <span className="text-[9px] font-black uppercase tracking-widest">Listening for incoming signals...</span>
                    </div>
                ) : (
                    messages.map((msg) => (
